...
from test_file import cv_commands, pac_commands, run_chi_macro
...
if mode == "PAC Analysis":
    ...
    with col1:
        ...
        time_delay=st.number_input("Time delay (s) (0 to 100)", min_value=0.0, max_value=100.0, value=0.01, step=0.01)
        sensitivity2=st.number_input("Sensitivity 2 (A/V) (1e-12 to 0.1)", min_value=1e-12, max_value=0.1, key="sensitivity2", format="%.2e")
    with col2:
        ...
        max_incr = st.number_input("Maximum increment for potential change (V) (1e-6 to 10)", min_value=1e-6, max_value=10.0,value = 0.01, step = 0.001, format="%.4f")
        ...
        iratioon = st.checkbox("Use current ratio for probe-stop")
        iabson = st.checkbox("Enable absolute current for probe-stop")
    ...
    if st.button("Submit Parameters"):
        ...
        "max_incr": max_incr, "withdraw": withdraw, "iratioon": iratioon, "iabson": iabson,
        "e2on": e2on, "epon": epon}
        st.write("Parameters submitted successfully!")
